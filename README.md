...
ヒートマップを生成するWebSocketサーバーでは [node-red](https://nodered.org/) を利用します。公式サイトの手順に従いインストールしてください。起動後に後述のnode-redフローをインポートしてください。
...
## node-redフロー
以下のプログラムをnode-redにインポートしてください。WebSocketで3次元データを受信し、位置情報、点群データ、3Dヒートマップに変換します。変換したデータは、HTTPで取得することができます。
